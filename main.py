...
def get_tianhang2():
    try:
        key = config["tian_api"]
        url = "http://api.tianapi.com/saylove/index?key={}".format(key)
        headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) '
                          'AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Safari/537.36',
            'Content-type': 'application/x-www-form-urlencoded'
        ...
        response = get(url, headers=headers).json()
        if response["code"] == 200:
            twqh = response["newslist"][0]["content"]
        else:
            twqh = ""
    except KeyError:
        twqh = ""
...
def send_message(to_user, access_token, region_name, weather, temp, wind_dir, note_ch, note_en, max_temp, min_temp,
                 sunrise, sunset, category, pm2p5, proposal, chp,twqh):
...
if __name__ == "__main__":
    ...
    for user in users:
        send_message(user, accessToken, region, weather, temp, wind_dir, note_ch, note_en, max_temp, min_temp, sunrise,
                     sunset, category, pm2p5, proposal, chp,twqh)
    os.system("pause")
